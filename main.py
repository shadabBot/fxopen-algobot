...
# ==========================================
# HMAC SIGNATURE (REQUIRED FOR AUTH)
# ==========================================
def generate_hmac_signature(payload, secret):
    timestamp = int(time.time() * 1000)  # ms
    message = json.dumps(payload, separators=(',', ':')) if isinstance(payload, dict) else payload
...
def api_request(method, endpoint, payload=None, params=None):
    ...
    if payload:
        sig, ts = generate_hmac_signature(payload, TOKEN_SECRET)
...
# ==========================================
# MAIN BOT LOOP — DETAILED CONDITIONS
# ==========================================
def run_bot():
    log("FXOPEN TICKTRADER BOT STARTED — LOGIN 28503612 — CONNECTING...")
    time.sleep(10)  # Render warm-up
    ...
    while True:
        try:
            ...
            send_email("BOT CONNECTED", f"Balance: ${acc['balance']:,.2f} | Login: 28503612")
            ...
            while True:
                try:
                    ...
                    log(f"EMA Trend: {'UP' if c['ema_f'] > c['ema_s'] else 'DOWN'} | HTF: {'BULL' if h['close'] > h['open'] else 'BEAR'}")
